...
/* ---------------------------------------------------------------------
   QUÉ BUSCAR · el perfil ideal, sin nombres propios (2026-09-18)
   --------------------------------------------------------------------- */
const fuenteScout = require('fs').readFileSync('./js/sgadd-scouting.js', 'utf8');
check('cada familia del catálogo tiene al menos dos atributos que buscar',
  S.CATALOGO_DEFENSOR.every(c => S.atributosDefensor(c.id).length >= 2),
  S.CATALOGO_DEFENSOR.map(c => c.id + ':' + S.atributosDefensor(c.id).length).join('|'));
check('y nunca más de cuatro: es una guía rápida',
  S.CATALOGO_DEFENSOR.every(c => S.atributosDefensor(c.id).length <= 4));
check('toda señal de los pesos tiene su frase de cancha (no queda un peso mudo)',
  S.CATALOGO_DEFENSOR.every(c => Object.keys(c.defiende).every(k => !!S.ATRIBUTOS_SEÑAL[k])));
check('ninguna frase lleva coma ni «y» adentro: se enumeran y la partirían',
  Object.keys(S.ATRIBUTOS_SEÑAL).every(k => Object.values(S.ATRIBUTOS_SEÑAL[k]).every(t => !/,| y /.test(t))));
const atlet = S.atributosDefensor('perimetralAtletico');
check('los atributos van del peso más fuerte al más débil',
  atlet[0] === S.ATRIBUTOS_SEÑAL.perimetral.mas && atlet[1] === S.ATRIBUTOS_SEÑAL.pr.mas, atlet.join(' | '));
/* El signo del peso cambia la frase: al Perimetral Físico las faltas le
   suman (es contacto), al Atlético le restan (tiene que contener sin fallar). */
check('las faltas con peso negativo se piden como disciplina',
  atlet.indexOf(S.ATRIBUTOS_SEÑAL.fc.menos) !== -1);
check('y con peso positivo, como contacto físico',
  S.atributosDefensor('perimetralFisico').indexOf(S.ATRIBUTOS_SEÑAL.fc.mas) !== -1);
check('al Referente de Zona se le piden tapas antes que rebote, igual que en sus pesos',
  S.atributosDefensor('referenteZona')[0] === S.ATRIBUTOS_SEÑAL.tc.mas);
check('los perfiles de contención perimetral no piden tapar',
  ['perimetralAtletico', 'especialistaPerimetral', 'contencionTactica']
    .every(id => S.atributosDefensor(id).indexOf(S.ATRIBUTOS_SEÑAL.tc.mas) === -1));
const guiaDrive = S.queBuscar('Contenedor de Penetraciones / Drive Containment');
check('queBuscar arma el texto en castellano, con mayúscula y «y» al final',
  !!guiaDrive && /^Desplazamiento lateral/.test(guiaDrive.texto) && / y [^,]+$/.test(guiaDrive.texto), guiaDrive && guiaDrive.texto);
check('y trae la tarea del perfil para el tooltip',
  !!guiaDrive && /primer paso/.test(guiaDrive.tarea));
check('un perfil que no es del catálogo (el DT escribió otra cosa) no inventa guía',
  S.queBuscar('MARINI') === null && S.queBuscar('') === null);
check('el motor puro no expone el recomendador de nombres',
  typeof S.candidatosPropios === 'undefined' && typeof S.señalesPlantel === 'undefined' && typeof S.compatiblePosicional === 'undefined');
check('ni la UI: la celda ya no dice «De los nuestros» ni nombra jugadores propios',
  !/De los nuestros/.test(fuenteScout) && !/marca\.candidatos/.test(fuenteScout));
check('la celda muestra «Qué buscar» y la columna se llama Perfil defensivo ideal',
  /Qué buscar:/.test(fuenteScout) && />Perfil defensivo ideal</.test(fuenteScout));
check('la nota dice que el nombre lo elige el cuerpo técnico',
  /Qué jugador de tu plantel<\/b> reúne esos atributos lo decide el cuerpo técnico/.test(fuenteScout));
/* Lo que el box score NO trae es el trabajo sin pelota, y es parte de por
   qué el nombre lo pone el cuerpo técnico. */
check('y aclara qué es lo que el box score NO mide',
  /trabajo sin pelota/.test(fuenteScout) && /no trae altura ni posición/.test(fuenteScout));
...
check('cada fila del informe trae el perfil ideal y su guía, sin candidatos',
  cruceA.jugadoresRival.filas.length > 0 && cruceA.jugadoresRival.filas.every(f =>
    !!f.marca.defensor && !!f.marca.queBuscar && f.marca.queBuscar.texto.length > 0 && f.marca.candidatos === undefined),
  cruceA.jugadoresRival.filas.map(f => f.marca.defensor).join(' | '));
check('la guía de cada fila es la de SU perfil, no una genérica',
  cruceB.jugadoresRival.filas.every(f => f.marca.queBuscar.texto === S.queBuscar(f.marca.defensor).texto));
...
/* =====================================================================
   LOS DADOS DE BAJA NO DEFIENDEN · plantelDefensor como plantelOrdenado
   ===================================================================== */
console.log('\n24. PLANTEL QUE DEFIENDE · sin los dados de BAJA');
console.log('═'.repeat(70));

const platense = S.plantelDefensor(idx, 'ATENAS A', 'PLATENSE A');
const dadoDeBaja = platense.length ? platense[0]['NOMBRES'] : null;
global.SGADD_BUZON = { enPlan: (n) => n !== dadoDeBaja };
const sinBaja = S.plantelDefensor(idx, 'ATENAS A', 'PLATENSE A');
const sinBajaRespaldo = S.plantelDefensor(idx, 'ATENAS A', null);
delete global.SGADD_BUZON;
check('plantelDefensor excluye al dado de BAJA en el cruce explícito',
  !!dadoDeBaja && sinBaja.length === platense.length - 1 && sinBaja.every(j => j['NOMBRES'] !== dadoDeBaja),
  platense.length + ' → ' + sinBaja.length);
check('y también en el respaldo por esEquipoPropio',
  sinBajaRespaldo.every(j => j['NOMBRES'] !== dadoDeBaja));
check('sin el módulo de estados entran todos, como antes',
  S.plantelDefensor(idx, 'ATENAS A', 'PLATENSE A').length === platense.length);
check('plantelOrdenado y plantelDefensor usan el MISMO criterio (una sola función)',
  /function plantelOrdenado[\s\S]{0,200}\.filter\(enPlan\)/.test(fuenteCruce)
  && (fuenteCruce.match(/SGADD_BUZON\.enPlan\(/g) || []).length === 1);
